...
/**
 * Customgr enrol plugin implementation.
 *
 ...
 * @since  Moodle 3.0
 */
function enrol_customgr_check_group_enrolment_key($courseid, $enrolpassword) {
    global $DB;
...
class enrol_customgr_enrol_form extends moodleform {
    protected $instance;
    ...
    public function definition() {
        ...
        if ($instance->password) {
            ...
            $keyholders = get_users_by_capability($context, 'enrol/customgr:holdkey', $ufields);
            $keyholdercount = 0;
            foreach ($keyholders as $keyholder) {
                $keyholdercount++;
                if ($keyholdercount === 1) {
                    $mform->addElement('static', 'keyholder', '', get_string('keyholder', 'enrol_customgr'));
                }
        ...
        $this->add_action_buttons(false, get_string('enrolme', 'enrol_customgr'));
