...
/**
 * Customgr enrolment plugin implementation.
 * @author     2024 David Herney @ Engagement
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_customgr_plugin extends enrol_plugin {
    ...
    public function get_info_icons(array $instances) {
        ...
        $icons = array();
        if ($nokey) {
            $icons[] = new pix_icon('withoutkey', get_string('pluginname', 'enrol_customgr'), 'enrol_customgr');
        }
        if ($key) {
            $icons[] = new pix_icon('withkey', get_string('pluginname', 'enrol_customgr'), 'enrol_customgr');
        }
        return $icons;
    }
    ...
    public function can_add_instance($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/customgr:config', $context)) {
            return false;
        }

        $groups = $DB->count_records('groups', ['courseid' => $courseid]);

        if (!$groups) {
            // The course does not have any groups.
            return false;
        }
     ...
     */
    public function enrol_customgr(stdClass $instance, $data = null) {
        global $DB, $USER, $CFG;
        ...
        if (!$instance->customint1) {
            // Thr group is required in the instance.
            return;
        }
        $group = $DB->get_record('groups', ['id' => $instance->customint1, 'courseid' => $instance->courseid], 'id', 'id');

        // Check it because the group can be deleted or exist in other course (bad copy).
        if (!$group) {
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        if (true === $enrolstatus) {
            ...
            $form = new enrol_customgr_enrol_form(null, $instance);
            $instanceid = optional_param('instance', 0, PARAM_INT);
            if ($instance->id == $instanceid) {
                if ($data = $form->get_data()) {
                    $this->enrol_customgr($instance, $data);
                }
            }
        } else {
            ...
            $form = new enrol_customgr_empty_form($url, $data);
        }
    ...
    public function can_self_enrol(stdClass $instance, $checkuserenrolment = true) {
        ...
        if ($checkuserenrolment) {
            ...
            if ($DB->get_record('user_enrolments', array('userid' => $USER->id, 'enrolid' => $instance->id))) {
                return get_string('canntenrol', 'enrol_customgr');
            }
        }
        ...
        if (!has_capability('enrol/customgr:enrolself', context_course::instance($instance->courseid))) {
            return get_string('canntenrol', 'enrol_customgr');
        }
    ...
    public function is_self_enrol_available(stdClass $instance) {
        ...
        if ($instance->enrolstartdate != 0 && $instance->enrolstartdate > time()) {
            return get_string('canntenrolearly', 'enrol_customgr', userdate($instance->enrolstartdate));
        }

        if ($instance->enrolenddate != 0 && $instance->enrolenddate < time()) {
            return get_string('canntenrollate', 'enrol_customgr', userdate($instance->enrolenddate));
        }
        ...
        if (empty($instance->customint1)) {
            return get_string('grouprequired', 'enrol_customgr');
        }
        ...
        if ($instance->customint3 > 0) {
            ...
            if ($count >= $instance->customint3) {
                ...
                return get_string('maxenrolledreached', 'enrol_customgr');
            }
        }
        ...
        if ($instance->customint5) {
            ...
            if (!cohort_is_member($instance->customint5, $USER->id)) {
                ...
                return markdown_to_html(get_string('cohortnonmemberinfo', 'enrol_customgr', $a));
            }
        }
    ...
    public function get_enrol_info(stdClass $instance) {
        ...
        if ($instance->password) {
            ...
            $instanceinfo->requiredparam->enrolpassword = get_string('password', 'enrol_customgr');
        }
        ...
        if ((true === $instanceinfo->status) && $instance->password) {
            $instanceinfo->wsfunction = 'enrol_customgr_get_instance_info';
        }
        return $instanceinfo;
    }
    ...
    protected function email_welcome_message($instance, $user) {
        ...
        if (!is_null($instance->customtext1) && trim($instance->customtext1) !== '') {
            ...
            $messagetext = get_string('welcometocoursetext', 'enrol_customgr', $a);
            $messagehtml = text_to_html($messagetext, null, false, true);
        }

        $subject = get_string('welcometocourse', 'enrol_customgr', format_string($course->fullname, true, array('context'=>$context)));
    ...
    public function sync(progress_trace $trace, $courseid = null) {
        ...
        $sql = "SELECT e.*, ue.userid
                  ...
                  JOIN {enrol} e ON (e.id = ue.enrolid AND e.enrol = 'customgr' AND e.customint2 > 0)
     ...
     * as defined by sort_by_roleassignment_authority() having 'enrol/customgr:manage'
     * capability.
     *
    ...
    protected function get_enroller($instanceid) {
        ...
        if ($users = get_enrolled_users($context, 'enrol/customgr:manage')) {
    ...
    public function can_delete_instance($instance) {
        $context = context_course::instance($instance->courseid);
        return has_capability('enrol/customgr:config', $context);
    }
    ...
    public function can_hide_show_instance($instance) {
        ...
        if (!has_capability('enrol/customgr:config', $context)) {
            return false;
        }
    ...
    public function get_bulk_operations(course_enrolment_manager $manager) {
        ...
        if (has_capability("enrol/customgr:manage", $context)) {
            $bulkoperations['editselectedusers'] = new enrol_customgr_editselectedusers_operation($manager, $this);
        }
        if (has_capability("enrol/customgr:unenrol", $context)) {
            $bulkoperations['deleteselectedusers'] = new enrol_customgr_deleteselectedusers_operation($manager, $this);
        }
    ...
    public function edit_instance_form($instance, MoodleQuickForm $mform, $context) {
        ...
        $mform->addElement('select', 'status', get_string('status', 'enrol_customgr'), $options);
        $mform->addHelpButton('status', 'status', 'enrol_customgr');

        $options = $this->get_newenrols_options();
        $mform->addElement('select', 'customint6', get_string('newenrols', 'enrol_customgr'), $options);
        $mform->addHelpButton('customint6', 'newenrols', 'enrol_customgr');
        $mform->disabledIf('customint6', 'status', 'eq', ENROL_INSTANCE_DISABLED);

        $passattribs = array('size' => '20', 'maxlength' => '50');
        $mform->addElement('passwordunmask', 'password', get_string('password', 'enrol_customgr'), $passattribs);
        $mform->addHelpButton('password', 'password', 'enrol_customgr');
        ...
        $mform->addElement('select', 'customint1', get_string('group', 'enrol_customgr'), $options);
        $mform->addHelpButton('customint1', 'group', 'enrol_customgr');

        $roles = $this->extend_assignable_roles($context, $instance->roleid);
        $mform->addElement('select', 'roleid', get_string('role', 'enrol_customgr'), $roles);

        $options = array('optional' => true, 'defaultunit' => 86400);
        $mform->addElement('duration', 'enrolperiod', get_string('enrolperiod', 'enrol_customgr'), $options);
        $mform->addHelpButton('enrolperiod', 'enrolperiod', 'enrol_customgr');
        ...
        $mform->addElement('date_time_selector', 'enrolstartdate', get_string('enrolstartdate', 'enrol_customgr'), $options);
        $mform->setDefault('enrolstartdate', 0);
        $mform->addHelpButton('enrolstartdate', 'enrolstartdate', 'enrol_customgr');

        $options = array('optional' => true);
        $mform->addElement('date_time_selector', 'enrolenddate', get_string('enrolenddate', 'enrol_customgr'), $options);
        $mform->setDefault('enrolenddate', 0);
        $mform->addHelpButton('enrolenddate', 'enrolenddate', 'enrol_customgr');

        $options = $this->get_longtimenosee_options();
        $mform->addElement('select', 'customint2', get_string('longtimenosee', 'enrol_customgr'), $options);
        $mform->addHelpButton('customint2', 'longtimenosee', 'enrol_customgr');

        $mform->addElement('text', 'customint3', get_string('maxenrolled', 'enrol_customgr'));
        $mform->addHelpButton('customint3', 'maxenrolled', 'enrol_customgr');
        ...
        if (count($cohorts) > 1) {
            $mform->addElement('select', 'customint5', get_string('cohortonly', 'enrol_customgr'), $cohorts);
            $mform->addHelpButton('customint5', 'cohortonly', 'enrol_customgr');
        } else {
        ...
        $mform->addElement('select', 'customint4', get_string('sendcoursewelcomemessage', 'enrol_customgr'),
                enrol_send_welcome_email_options());
        $mform->addHelpButton('customint4', 'sendcoursewelcomemessage', 'enrol_customgr');

        $options = array('cols' => '60', 'rows' => '8');
        $mform->addElement('textarea', 'customtext1', get_string('customwelcomemessage', 'enrol_customgr'), $options);
        $mform->addHelpButton('customtext1', 'customwelcomemessage', 'enrol_customgr');
    ...
    public function edit_instance_validation($data, $files, $instance, $context) {
        ...
        if ($data['status'] == ENROL_INSTANCE_ENABLED) {
            if (!empty($data['enrolenddate']) and $data['enrolenddate'] < $data['enrolstartdate']) {
                $errors['enrolenddate'] = get_string('enrolenddaterror', 'enrol_customgr');
            }
        }
    ...
    public function get_welcome_email_contact($sendoption, $context) {
        ...
        if ($sendoption == ENROL_SEND_EMAIL_FROM_COURSE_CONTACT) {
            ...
            // Send as the first user with enrol/customgr:holdkey capability assigned in the course.
            list($sort) = users_order_by_sql('u');
            $keyholders = get_users_by_capability($context, 'enrol/customgr:holdkey', 'u.*', $sort);
            if (!empty($keyholders)) {
 ...
 */
function enrol_customgr_get_fontawesome_icon_map() {
    return [
        'enrol_customgr:withkey' => 'fa-key',
        'enrol_customgr:withoutkey' => 'fa-sign-in',
    ];
}
